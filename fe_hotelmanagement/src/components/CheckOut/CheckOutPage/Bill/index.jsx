...
const BillContainer = ({
  ...
  const originalPrice = reservationDetails?.reduce(
    (total, reservationDetail) => {
      return (
        total + reservationDetail.price * reservationDetail.quantity * days
      );
    },
    0
  );
  ...
  return (
    <div className="bill-container">
      <Container
        className="mt-4 p-3 detail-section"
        style={{ backgroundColor: "rgb(248, 243, 243)" }}
      >
        <h5 className="mt-1">
          <Badge bg="primary">Hóa đơn đã tính toán</Badge>
        </h5>

        {reservationDetails?.map((reservationDetail) => {
          return (
            <div className="d-flex justify-content-between mt-3 mb-3">
              <h6 className="text-start mb-0">
                Giá gốc ({reservationDetail.quantity} phòng{" "}
                {reservationDetail.name} x {days} đêm)
              </h6>
              <h6 className="text-end mb-0">
                {formatCurrency(
                  reservationDetail.quantity * reservationDetail.price * days
                )}
                ₫
              </h6>
            </div>
            ...
            <div className="d-flex justify-content-between mt-3 mb-3 discount-section">
              ...
              <h6 className="text-end mb-0">
                {Math.min(
                  ...
                  (originalPrice * discount.discountPrecentage) / 100
                )}
                ₫
              </h6>
            </div>
      ...
      <Container className="p-3 summary-section">
        <div className="d-flex justify-content-between">
          ...
          <h5 className="text-end mb-0 fw-semibold">
            {formatCurrency(totalPrice)} ₫
          </h5>
        </div>
        <hr
          style={{ border: "none", borderTop: "1px dashed rgb(248, 243, 243)" }}
        ></hr>
        <h6 className="text-start mb-0 fw-light">
          Giá đã bao gồm:{" "}
          <span className="fw-light">Phí dịch vụ 5% và ghi chú đặt phòng</span>
        </h6>
      </Container>
    </div>
  );
